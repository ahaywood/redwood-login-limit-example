...
export const handler = async (
  ...
  const loginOptions: DbAuthHandlerOptions['login'] = {
    ...
    handler: (user) => {
      // TODO: Reset the login attempts count and lockedAt time

      // if the user is locked for trying too many times, throw an error.
      if (user.unlockedAt && isBefore(new Date(), new Date(user.unlockedAt))) {
        throw new Error('User is locked. Try again later.')
      }
